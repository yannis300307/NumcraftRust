...
const SCREEN_WIDTH: isize = 340;
const SCREEN_HEIGHT: isize = 240;
const HALF_SCREEN_WIDTH: isize = SCREEN_WIDTH/2;
const HALF_SCREEN_HEIGHT: isize = SCREEN_HEIGHT/2;
...
impl Renderer {
    ...
    fn compute_transform(&self, point: Vector3<f32>) -> Vector2<f32> {
        ...
        let display_surface_position: Vector3<f32> = Vector3::new(1.0, 1.0, 0.25);
    ...
    fn draw_2d_triangle(tri: Triangle2d) {
        draw_line(tri.p1.x as isize + HALF_SCREEN_WIDTH, tri.p1.y as isize + HALF_SCREEN_HEIGHT, tri.p2.x as isize + HALF_SCREEN_WIDTH, tri.p2.y as isize + HALF_SCREEN_HEIGHT, get_color(0b11111, 0b0, 0b0));
        draw_line(tri.p2.x as isize + HALF_SCREEN_WIDTH, tri.p2.y as isize + HALF_SCREEN_HEIGHT, tri.p3.x as isize + HALF_SCREEN_WIDTH, tri.p3.y as isize + HALF_SCREEN_HEIGHT, get_color(0b11111, 0b0, 0b0));
        draw_line(tri.p3.x as isize + HALF_SCREEN_WIDTH, tri.p3.y as isize + HALF_SCREEN_HEIGHT, tri.p1.x as isize + HALF_SCREEN_WIDTH, tri.p1.y as isize + HALF_SCREEN_HEIGHT, get_color(0b11111, 0b0, 0b0));
    }
    ...
    pub fn draw_debug_float(value: f32) {
        ...
        eadk::display::draw_string(s, eadk::Point{x: 10, y : 10}, false, get_color(0b11111, 0, 0), get_color(0b11111, 0b111111, 0b11111));
    }

    pub fn update(&self) {
        Renderer::clear_screen(get_color(0b01111, 0b011111, 0b11111));
    
        self.draw_3d_triangle(Triangle3d { p1: Vector3::new(-1.0, -1.0, -1.0), p2: Vector3::new(1.0, 1.0, -1.0), p3: Vector3::new(1.0, -1.0, -1.0) });
